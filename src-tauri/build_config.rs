...
pub fn should_use_check_config(root: &Path, profile: &str, has_tauri_config: bool) -> bool {
    profile == "debug"
        && !has_tauri_config
...
mod tests {
    ...
    fn complete_windows_runtime_keeps_the_regular_tauri_config() {
        ...
        assert!(super::should_use_check_config(&root, "debug", false));
        assert!(!super::should_use_check_config(&root, "release", false));
        assert!(!super::should_use_check_config(&root, "debug", true));
