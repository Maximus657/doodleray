...
| Authenticate updater artifacts. | Covered in CI gate | Release workflow requires the Tauri updater private key, emits the NSIS updater signature, and verifies exact artifact SHA-256 values. Windows Authenticode is intentionally not used. | Production readiness requires updater-signed CI bytes and the clean-OS/update matrix; Windows certificate status remains informational. |
...
| Microsoft / Tauri / WebView2 docs | Offline WebView2 installer mode; WinINet vs WinHTTP inspected separately; NCSI treated as warning; mandatory Tauri updater signing; perMachine NSIS with service repair hooks. | yes | WebView2/VC++/WinHTTP/NCSI in deep snapshot; updater-signature + exact-hash workflow checks | Exercise the consolidated CI workflow with updater signing and complete the OS matrix; MSI remains out of scope. |
...
6. Run the updater-signed CI build and verify identical QA/GitHub/CDN hashes before production promotion.
