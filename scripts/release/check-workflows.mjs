...
export function checkReleaseWorkflows(root) {
  ...
  if (!/TAURI_SIGNING_PRIVATE_KEY/.test(release) || !/FromBase64String\(\$key\)/.test(release) || !/tauri signer sign/.test(release) || !/\.sig/.test(release)) errors.push('Tauri updater signing and signatures must fail closed');
